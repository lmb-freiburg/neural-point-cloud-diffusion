...
#
# Modified 2023 by Philipp Schroeppel. Modifications include: remove things that are not required
# in this project and replace the visualization of numpy arrays.
...
def setup_writers(log_wandb, log_tensorboard, max_iterations=None, tensorboard_logs_dir=None, wandb_logs_dir=None, exp_id=None, config=None, comment=None, project="npcd"):
    ...
    if log_wandb:
        ...
        curr_writer = WandbWriter(project=project, log_dir=wandb_logs_dir, exp_id=exp_id, config=config, comment=comment)
...
class WandbWriter(Writer):
    ...
    def __init__(self, log_dir, project="npcd", exp_id=None, config=None, comment=None):
        wandb.init(project=project, dir=str(log_dir), reinit=True, resume=True, config=config, name=exp_id, notes=comment)
